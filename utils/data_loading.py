...
class Data_Loader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        category_features=pateint_tabular[['F','M','Venous drainage','Hemorrhage']].copy()
        category_features=torch.Tensor(np.array(category_features))

        continuous_features=pateint_tabular[['Age at GK','RV','CSF比例','腦組織比例','血管比例','CSF體積(ml)','腦組織體積(ml)','血管體積(ml)','TC(Gy)','TP(Gy)']].copy()
        continuous_features=torch.Tensor(np.array(continuous_features))
