...
def start_train(model, device, info):
    ...
    for epoch in range(1, info['epochs'] + 1):
        ...
        for batch_idx, batch in progress_bar:
            images, target, ID, continuous_features, category_features = batch[
                'image'], batch['target'], batch['ID'], batch[
                    'continuous_features'], batch['category_features']

            images = images.to(device=device, dtype=torch.float32)
            target = target.to(device=device, dtype=torch.float32).view(-1, 1)
            continuous_features = continuous_features.to(device=device,
                                                         dtype=torch.float32)

            category_features = category_features.to(device=device,
                                                     dtype=torch.int32)
            category_features = torch.squeeze(category_features)
            continuous_features = torch.squeeze(continuous_features)

            images = images.unsqueeze(1)
            pred = model(images, category_features, continuous_features)
        ...
        if epoch % 1 == 0:
            ...
            for batch in val_loader:
                ...
                continuous_features = continuous_features.to(
                ...
                with torch.no_grad():
                    pred = model(images, category_features,
                                 continuous_features)
            ...
            auc = roc_auc_score(all_targets, all_predictions_proba)
            ...
            plot_confusion_matrix(all_targets, optimal_predictions, info,
                                  epoch)

            writer.add_scalar('Validation Accuracy', accuracy, epoch)
            writer.add_scalar('Validation F1 Score', f1_optimal, epoch)
            print(
                f'Epoch {epoch} - Validation Accuracy: {accuracy:.4f}, Validation F1 Score: {f1_optimal:.4f},AUC:{auc}'
            )
            ...
            if auc >= best_auc:
                ...
                torch.save(
                    model.state_dict(),
                    f'./experiment/{info["target"]}/fold{info["fold"]}/{info["exp_time"]}/checkpoints/best_model.pth'
                )
                with open(
                        f'./experiment/{info["target"]}/fold{info["fold"]}/{info["exp_time"]}/checkpoints/threshold.txt',
                        'w') as file:
                    file.write(f'{optimal_threshold}\n')

            torch.save(
                model.state_dict(),
                f'./experiment/{info["target"]}/fold{info["fold"]}/{info["exp_time"]}/checkpoints/epoch/epoch{epoch}.pth'
            )
            with open(
                    f'./experiment/{info["target"]}/fold{info["fold"]}/{info["exp_time"]}/checkpoints/epoch/threshold.txt',
                    'a') as file:
...
def start_test(device, info):
    ...
    best_model = CombinedModel(1, 1, 64, 64)
    ...
    for batch in test_loader:
        with torch.no_grad():
            images, target, ID, continuous_features,category_features = batch['image'], batch['target'], batch['ID'], batch['continuous_features'],batch['category_features']

            images = images.to(device=device, dtype=torch.float32)
            target = target.to(device=device, dtype=torch.float32).view(-1, 1)
            continuous_features= continuous_features.to(device=device, dtype=torch.float32)

            category_features= category_features.to(device=device, dtype=torch.int32)
            category_features=torch.squeeze(category_features)
            continuous_features=torch.squeeze(continuous_features)
            ...
            pred = best_model(images,category_features,continuous_features)
